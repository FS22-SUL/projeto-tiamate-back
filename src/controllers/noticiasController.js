const { prisma } = require("../utils"); //noticias
    ...
    try {
        ...
        const resultado = new Promise((resolve, reject) => {
            form.parse(req, async (error, fields, files) => {
                ...
                const filenameOriginal = files.noticia_imagem[0].originalFilename;
                ...
                const oldpath = files.noticia_imagem[0].filepath;
    ...
    try {
        ...
        const resultado = new Promise((resolve, reject) => {
            form.parse(req, async (error, fields, files) => {
                ...
                if (files.noticia_imagem && files.noticia_imagem[0]) {
                    const file = files.noticia_imagem[0];
                    ...
                    imagemUrl = `${req.protocol}://${req.headers.host}/uploads/noticias/${newFilename}`;
                }
...
async function deletar(id) {
    try {
        const req = await prisma.noticias.delete({
            where: {
                noticia_id: Number(id),
            },
        });
